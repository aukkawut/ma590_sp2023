...
$$\begin{aligned}
\frac{\text{cov}(A,B)}{\text{cov}(A,C)} &= \frac{\mathbb{E}[AB]-\mathbb{E}[A]\mathbb{E}[B]}{\mathbb{E}[AC]-\mathbb{E}[A]\mathbb{E}[C]} \\
&= \frac{\mathbb{E}[AB] - p\mathbb{E}[B]}{\mathbb{E}[AC] - p\mathbb{E}[C]}\\
\end{aligned}$$
...
- $w(0)=1, w(1)=0$: Students who requested help in non-video condition
- $w(0)=0, w(1)=1$: Students who requested help in video condition
...
If monotonicity is assumed, one can say that there is no defiers in the dataset. 
In $(w,z) = (0,0)$, there are both compliers and never test. In comparison to $(w,z) = (1,0)$, which only 
has never test. 